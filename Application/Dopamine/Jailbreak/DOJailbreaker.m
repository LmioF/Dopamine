...
#import "DORoothidePatchfinder.h"
...
- (NSError *)gatherSystemInformation
{
    ...
    if (r == 0) {
        ...
        if (_systemInfoXdict && roothide_add_kernel_offsets(_systemInfoXdict) != 0) {
            _systemInfoXdict = nil;
        }
        if (_systemInfoXdict) {
        ...
        if (!_systemInfoXdict) {
            NSString *message = [NSString stringWithFormat:@"XPF failed with error: (%s)", xpf_get_error()];
            xpf_stop();
            return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedKernelPatchfinding userInfo:@{NSLocalizedDescriptionKey:message}];
        }
        xpf_stop();
    }
    else {
