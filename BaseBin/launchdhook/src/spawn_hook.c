...
{
	kern_return_t kr = litehook_hook_function(__posix_spawn, roothide_launchd___posix_spawn_prehook);
	if (kr != KERN_SUCCESS) launchd_panic("Failed to install roothide spawn hook: %d", kr);
}
