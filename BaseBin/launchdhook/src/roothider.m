...
void roothide_launchd_postinit(bool firstLoad)
{
	...
	if (__builtin_available(iOS 16.0, *))
	{
		kern_return_t kr = litehook_hook_function(__sysctl, __sysctl_hook);
		if (kr != KERN_SUCCESS) launchd_panic("Failed to install roothide sysctl hook: %d", kr);
		kr = litehook_hook_function(__sysctlbyname, __sysctlbyname_hook);
		if (kr != KERN_SUCCESS) launchd_panic("Failed to install roothide sysctlbyname hook: %d", kr);
		orig_bind = bind;
		litehook_rebind_symbol(LITEHOOK_REBIND_GLOBAL, bind, new_bind, NULL);
	}
#ifdef __arm64e__
	else 
	{
	...
	litehook_rebind_symbol(LITEHOOK_REBIND_GLOBAL, xpc_dictionary_create_reply, new_xpc_dictionary_create_reply, NULL);
...
{
    const char *image = "/System/Library/Frameworks/IOSurface.framework/IOSurface";
    io_service_t* __iosService = litehook_find_dsc_symbol(image, "__iosService");
    io_connect_t* __iosConnect = litehook_find_dsc_symbol(image, "__iosConnect");
...
int roothide_launchd___posix_spawn_prehook(pid_t *restrict pidp, const char *restrict path, struct _posix_spawn_args_desc *desc, char *const argv[restrict], char *const envp[restrict])
{
	...
	if (strcmp(path, JBROOT_PATH("/basebin/hookd")) == 0) {
		return __posix_spawn_orig_wrapper(pidp, path, desc, argv, envp);
	}
